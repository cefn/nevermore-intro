...
let quota = REQUESTS_PER_SECOND;

async function replenishQuota() {
  for (;;) {
    await sleep(1000);
...
// run quota process in background
replenishQuota();
...
try {
  ...
  for await (const settlement of retrievedNumberSequence) {
    if (settlement.status === "fulfilled") {
      ...
      console.error(
        `Retrieval of ${settlement.job.numberToRetrieve} failed for reason: ${settlement.reason}`,
      );
    }
  }
